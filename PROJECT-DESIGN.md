...
The current 0.3.7 release combines the protected-routing and exact-primary foundation with shorter bounded recovery after completed work, incomplete buffers, preparation, locker return, and saved-outfit restoration. Automatic idle return is tracked separately from explicit Recall so newly available protected work can retain a complete managed outfit instead of restoring and immediately re-equipping. Existing queued restoration jobs survive connective compatibility waits; safe targetless idle waits with missing gear use exterior egress; sequential incompatible-rule handoffs are decided from the pawn's actual current cell; and exact AOM transition jobs cannot use unrelated protected areas as shortcuts. Expanded and compact buffer displays both report completed tasks, and a work proposal that never takes control cannot erase completed progress after return is committed. The release candidate has been exercised on small and large heavily modded maps with Detailed logging, persisted snapshots, concurrent workers, Simple Sidearms, animal and robot saved-weapon hauling, tattered-apparel replacement, protected sleep, and repeated buffers without AOM exceptions, path or reservation failures, prolonged Standing, queue growth, or rapid-job-loop warnings. Focused synthetic save/load at every transition boundary, a deliberately staged incompatible cross-area handoff, and forced multi-pawn locker-cell contention were not separately reproduced; they remain disclosed validation limits rather than known faults. Manual priority and pawn-eligibility prototypes remain deferred; ammunition, inventory sidearms, offhands, shields, and automatic drafted switching are outside this weapon scope.
