...
The current 0.3.6 release combines exact primary-weapon requirements with the tested Phase 2 foundation, successful-completion buffers, condition and quality standards, tattered saved-apparel improvement, visitor departures, hazardous-route protection retention, gravship area continuity, Simple Sidearms, and large modded colonies, plus player-configurable log levels and a lower-overhead management window. Gameplay path requests for jobs whose concrete targets are outside managed areas now assign those cells a strong avoidance cost, so native work, hauling, wandering, and recreation prefer exterior routes instead of using a protected area as a shortcut; if crossing remains necessary, the ordinary boundary guard still prepares permitted actors, while actors already inside retain an uncustomized route out. Autonomous recreation is governed by Wandering access. Late-bound boundary handoffs retain the first exact interrupted native job across replacement thinker jobs and temporary reservation conflicts. Automatic weapon-mod selections remain compatible but no longer waive a rule's weapon requirement; only an explicit player-forced weapon choice does so. The release has been exercised on large and small modded maps with Detailed logging, repeated nested-rule handoffs, save/load continuity, animals and hauling bots, and outside-target route observation without path, Standing, reservation, or rapid-job-loop failures. Manual priority and pawn-eligibility prototypes remain deferred; ammunition, inventory sidearms, offhands, shields, and automatic drafted switching are outside this weapon scope.
