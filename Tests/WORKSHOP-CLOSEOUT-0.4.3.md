...
- Public content handle and refreshed subscriber manifest: **818263818669951996**.
- The public description includes the 0.4.3 Latest update block, child access and ceremony preparation. The separate [change note](https://steamcommunity.com/sharedfiles/filedetails/changelog/3792731788) includes Simpler child access, Prepare outfits before ceremonies, Smoother gathering and Existing saves. The retrieved public description and rendered note are preserved locally, and WORKSHOP.md has been aligned with that published copy.
- **12 files, 1,053,123 bytes**: every subscriber path, length and SHA-256 matches the audited stage. The public byte count and subscriber manifest also agree.
- DLL SHA-256: `149A319E8D6BDAB4494AD6727AA3607D3974DAE2BC2BDE1A6526BB18A005DF1D`.
- Harmony remains required and all eight gallery image IDs are unchanged. No new gallery captures are claimed.

## Restored development setup

RimWorld was closed when the installed junction was restored from the upload stage to `C:/GitHub/AutomaticOutfitManager`. Its DLL matches the published 0.4.3 runtime. The live About version and description were aligned with 0.4.3 after backing up the original, preserving the package identity and other metadata. Future staging continues from the RC worktree; the separate dirty live source was not synchronized.

## Prepared GitHub asset

The ZIP was created directly from the audited stage. Its entry set and all decompressed bytes match the twelve staged files. No GitHub upload, commit, tag, merge or push was performed during this closeout.

- ZIP: `AutomaticOutfitManager-0.4.3.zip`, **620,402 bytes**.
- ZIP SHA-256: `0646569C1F6F2710223BA781C7542750650F16D56DFDCA87B102680CF9F0F50E`.
- Uncompressed content: **1,053,123 bytes**.
