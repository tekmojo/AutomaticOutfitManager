...
try {
    ...
    & $compiler /nologo /target:exe /langversion:latest /warn:0 "/out:$testOutput" "/reference:$harmonyCopy" (Join-Path $PSScriptRoot 'PreparedMealLifetimeTests.cs') (Join-Path $rcRoot 'Source/Detection/PreparedIngestRetryRegistry.cs') (Join-Path $rcRoot 'Source/Patches/PreparationJobHandoff.cs')
    if ($LASTEXITCODE -ne 0) { throw 'Prepared meal lifetime compilation failed.' }
    & $testOutput
    if ($LASTEXITCODE -ne 0) { throw 'Prepared meal lifetime checks failed.' }
} finally {
