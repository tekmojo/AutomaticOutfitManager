param([switch]$PreviousDecision, [switch]$PreviousRouting, [switch]$PreviousConstruction)
...
'@
    ...
    if ($PreviousDecision -or $PreviousRouting -or $PreviousConstruction) {
        $previousPreference = $ErrorActionPreference
        $ErrorActionPreference = 'Continue'
        $output = & $testOutput 2>&1
        $result = $LASTEXITCODE
        $ErrorActionPreference = $previousPreference
        $expected=if($PreviousConstruction){'current adjacent delivery destination reaches boundary with PPE rule retained elsewhere'}else{'allowed empty Work rule does not block construction or transit'}
