...
$testOutput = Join-Path $env:TEMP ('aom-area-permission-tests-' + [Guid]::NewGuid().ToString('N') + '.exe')
try {
    & $compiler /nologo /target:exe /langversion:latest /warn:0 "/out:$testOutput" (Join-Path $PSScriptRoot 'AreaActivityPermissionsTests.cs') (Join-Path $rcRoot 'Source/Detection/AreaActivityPermissions.cs')
