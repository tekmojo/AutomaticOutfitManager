...
$testDir = Join-Path $env:TEMP ('aom-exit-tests-' + [Guid]::NewGuid().ToString('N'))
$testOutput = Join-Path $testDir 'AccessExitTests.exe'
...
try {
    ...
    & $compiler /nologo /target:exe /langversion:latest /warn:0 "/out:$testOutput" "/reference:$harmonyCopy" (Join-Path $PSScriptRoot 'AccessExitJobTests.cs') (Join-Path $rcRoot 'Source/Patches/AccessExitJobs.cs')
    if ($LASTEXITCODE -ne 0) { throw 'Access exit compilation failed.' }
    & $testOutput
    if ($LASTEXITCODE -ne 0) { throw 'Access exit contracts failed.' }
} finally {
